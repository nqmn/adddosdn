...
def run_attack(attacker_host, victim_ip, duration):
    attack_logger.info(f"Starting UDP Flood from {attacker_host.name} to {victim_ip} for {duration} seconds.")
    ...
    attack_logger.info(f"UDP Flood from {attacker_host.name} to {victim_ip} finished.")
    return process