...
def run_attack(attacker_host, victim_ip, duration):
    attack_logger.info(f"Starting ICMP Flood from {attacker_host.name} to {victim_ip} for {duration} seconds.")
    ...
    attack_logger.info(f"ICMP Flood from {attacker_host.name} to {victim_ip} finished.")
    return process