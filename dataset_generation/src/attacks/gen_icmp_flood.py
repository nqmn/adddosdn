...
def run_attack(attacker_host, victim_ip, duration):
    ...
    attack_logger.info(f"[icmp_flood] [Run ID: {run_id}] Starting Enhanced ICMP Flood from {attacker_host.name} to {victim_ip} for {duration} seconds.")
    ...
    attack_logger.info(f"[icmp_flood] [Run ID: {run_id}] Enhanced Features: Human-like timing, protocol compliance, network delay simulation")
    ...
    # Start enhanced ICMP flood with human-like timing
    attack_logger.debug(f"[icmp_flood] [Run ID: {run_id}] Starting enhanced ICMP packet generation with human-like timing")
    
    # Create a more sophisticated attack process that uses enhanced timing
    enhanced_scapy_cmd = f"""
import time
import random
from scapy.all import *

def enhanced_icmp_flood():
    ...
    while True:
        try:
            # Create ICMP packet with slight variations
...
"""
    ...
    attack_logger.info(f"[icmp_flood] [Run ID: {run_id}] Enhanced ICMP flood process started (PID: {process.pid})")
        ...
        if current_time >= next_monitor:
            ...
            try:
                if process.poll() is None:
                    ...
                    attack_logger.info(f"[icmp_flood] [Run ID: {run_id}] Attack progress: {elapsed:.1f}s elapsed, ~{estimated_packets} packets sent, Rate: {estimated_packets/elapsed:.1f} pps{phase_info}")
    ...
    attack_logger.info(f"[icmp_flood] [Run ID: {run_id}] Enhanced ICMP Flood from {attacker_host.name} to {victim_ip} finished.")
    ...
    attack_logger.info(f"[icmp_flood] [Run ID: {run_id}] Attack method: Enhanced ICMP Flood with Human-like Timing")
    attack_logger.info(f"[icmp_flood] [Run ID: {run_id}] Enhancement features: Human timing patterns, protocol compliance, network delay simulation")
